...
function [ xls ] = xlsopen (filename, xwrite=0, reqinterface=[], verbose=false)
  ...
  ftype = 0;
  has_suffix = 1;
  [sfxpos, ~, ~, ext] = regexpi (filename, '(\.xls[bmx]?|\.gnumeric|\.ods|\.scx|\.csv)');
  if (! isempty (sfxpos))
    ext = lower (ext{end});
    ...
    if (sfxpos(end) <= length (filename) - length (ext))
      ...
      ## If xwrite = 0, check file suffix, else add .xlsx
      has_suffix = 0;
    else
      switch ext
        ...
        case ".csv"                       ## csv. Detected for xlsread afficionados
          ftype = 6;
        otherwise
          ## FIXME here should come the extraneous ones like .fods, .uos, .wk1, ...
  ...
  if (fid < 0)                      ## File doesn't exist...
    if (! xwrite)                   ## ...which obviously is fatal for reading...
      ...
      if (fid < 0)                  ## File didn't exist yet. Simply create it
        ...
        error (sprintf ("xlsopen: write mode requested but file %s is not \
writable\n", filename))
      endif
    endif
  else
...
function fname = mtchext (fname)
  ...
  ext = find (ismember (prefext, exts))(1);
  fname = [fname ext];
