...
## Convert any @var{obj} into a compact JSON string.
##
## Input arguments:
##
## @itemize
## @item
## @var{obj} is an object to be converted to a JSON string.  There are no
## limitation on the class: double, float, int, logical, complex, char,
## etc.  Except for char and logical, however, all numerical data are
## treated as double precision floating point.
##
## Complex number, @var{c}, is converted to JSON object as follows:
## @verbatim
##        '{"re": real(C), "im": imag(C)}'
## @end verbatim
## @end item
##
## @item
## @var{prec} (optional argument) specifies the number of significant digits
## for numerical conversion.  The default value for optional numeric
## @var{prec} is 15.
## @end item
##
## @item
## @var{compact} (also optional) specifies the compactness when converting
## structured number arrays, such as complex numbers or numerical
## coordinates.  The default value is FALSE.
##
...
## @itemize
## @item
## Leaving @var{compact} as FALSE converts the above struct to a 
...
##
## @item
## while setting it to TRUE converts the above struct to a JSON 
## object of arrays, e.g:
## @verbatim
##        '{ "x": [[s.x]], "y": [[s.y]] }'
## @end verbatim
## @end item
## @end itemize
##
## The former might be preferred for expressiveness, however, the latter should be
## considered for compactness and speed.
##
## @end item
## @end itemize
##
## NOTE 1 @*
## The bodies of inline functions are stored as string; however, reference to
## external values will be lost. Consider the following inline function with
## external reference 'c',
## @verbatim
##      @(a) a + c
## @end verbatim
##
## The encoding will turn the above to string "@@(a) a+c", but the value
## of 'c' is lost in conversion.
##
## NOTE 2 @*
## JSON strings within OBJ (e.g. struct or cell arrays) are left unchanged.
##
## NOTE 3 @*
## Other classes, such as Java and COM objects, are merely referenced by 
## classname, along the lines of @code{"[java.lang.Double]"} and 
## @code{"[octave_com_object]"}.
##
## @seealso{fromJSON}
## @end deftypefn
...
function [str] = _tojson_ ( obj, opt );
  ...
  elseif( ischar(obj) );
      ...
      elseif ~( numel( str=strtrim(obj) ) && strfind('{}[]""',str([1,end])) );
        str = ['"' _rep_(obj,'"','\"') '"'];
      end
